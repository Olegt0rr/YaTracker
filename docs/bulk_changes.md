...
Пустой список задач приведёт к `ValueError`.
...
- `**kwargs` — то же самое, что и `values`, но именованными параметрами; значения из
  `kwargs` имеют приоритет при пересечении ключей. Итоговый набор полей (`values` +
  `kwargs`) не может быть пустым — иначе `ValueError`.
...
- `queue` — очередь назначения: ключ строкой либо объект `Queue` (тогда берётся `.key`).
...
Возвращает актуальное состояние операции по её `id`.

## Ошибки по каждой задаче — `get_bulk_change_issues`

Если операция завершилась статусом `FAILED` (частично или полностью), узнать, какие именно
задачи не удалось обработать и почему, можно так:
...
- `bulk_change` — `id` операции строкой либо уже полученный объект `BulkChange`.
- `interval` — пауза между опросами статуса, в секундах (должен быть больше `0`, иначе
  `ValueError`).
- `timeout` — необязательный общий тайм-аут ожидания, в секундах. Если операция не
  завершится за это время, будет выброшено исключение `TimeoutError`.
...
!!! note "Задержка появления операции"
    ...
    операцию действительно не найденной.
...
`BulkChangeError` содержит `errors` (словарь `поле: текст ошибки`) и `error_messages`
(список общих сообщений об ошибке, не привязанных к конкретному полю).
