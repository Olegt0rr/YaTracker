...
5. `gap_id` — идентификатор, который будет присвоен записи (не больше 128 символов).
   Если не передать, Трекер сгенерирует его сам. Отправляется как `id`.
...
1. `gaps` — записи об отсутствии, не больше 100. Каждая запись — словарь с ключами
...
!!! warning "Лимит в 100 записей"
    ...
    срабатывает раньше и с понятным сообщением.
...
1. `users` — логины или идентификаторы сотрудников, не больше 100. Одиночный логин
   вместо последовательности (`users="login1"`) бросает `TypeError`: иначе строка
   была бы разобрана по символам.
...
1. `gap_ids` — идентификаторы записей об отсутствии, не больше 100, каждый не длиннее
   128 символов. Отправляются одним query-параметром `gapIds`, через запятую.
   Одиночный идентификатор вместо последовательности (`gap_ids="6834..."`) бросает
...
!!! warning "Лимит в 100 записей"
    ...
    Если передать больше 100 идентификаторов, `delete_gaps` бросит `ValueError`, не
    обращаясь к API.
