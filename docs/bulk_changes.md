...
Пустой список задач приведёт к `ValueError`; пустая строка-фильтр в `bulk_update_issues` —
тоже `ValueError`, а строка вместо списка в двух других методах — `TypeError`.
...
- `**kwargs` — то же самое, что и `values`, но именованными параметрами; значения из
  `kwargs` имеют приоритет при пересечении ключей (ключи обеих форм кодируются одинаково,
  так что `values={"story_points": 1}, story_points=2` отправит одно поле `storyPoints`
  со значением `2`). Значения `None` в `kwargs`
...
- `timeout` — необязательный общий тайм-аут ожидания, в секундах (если задан, должен
  ...
  3.10). Дедлайн проверяется между запросами статуса, поэтому уже отправленный запрос
  не прерывается; сетевые ошибки и тайм-ауты самого HTTP-клиента пробрасываются как
  есть, а не маскируются под `TimeoutError` ожидания. По умолчанию тайм-аута нет —
  в автоматических сценариях лучше задавать его явно.

!!! note "Задержка появления операции"

    Сразу после создания операция может ещё не быть доступна по своему `id` — API Трекера
    в первые мгновения может ответить `404`. `wait_bulk_change` учитывает это: за всё
    время ожидания молча переживается до 10 ответов `404` (бюджет не восстанавливается
    успешными ответами), после чего выбрасывается `ObjectNotFoundError`.
