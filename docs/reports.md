...
4. `query`, `filter_`, `filter_id` — три взаимоисключающих способа задать критерии поиска
   задач для отчёта (см. предупреждение ниже):
...
!!! warning "`query`, `filter_` и `filter_id` взаимоисключающие"

    Можно передать только один из трёх параметров — метод сам это проверяет и бросает
    `ValueError`, если задано больше одного: сам API не поддерживает одновременное
    использование нескольких способов фильтрации.
