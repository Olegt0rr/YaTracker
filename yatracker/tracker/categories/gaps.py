...
def _encode_gap(gap: Mapping[str, Any]) -> dict[str, Any]:
    ...
    moments accept a `datetime` or a bare `date` as well as a ready-made
    API string.
    """
    encoded = {
...
class Gaps(BaseTracker):
    ...
    async def create_gap(
        ...
        *,
        ...
        :param from_: start of the absence: a datetime, a bare date
            (rendered as midnight UTC) or a ready-made API string. Must
            be less than `to`.
        :param to: end of the absence, in the same forms as `from_`.
        :param id_: id to assign to the record, sent as `id` (the
            trailing underscore keeps the name out of the way of the
            builtin, like `type_`). At most 128 characters; generated by
            the API when omitted.
    ...
    async def create_gaps(
        ...
        :param gaps: absence records to create, at most 100 (any
            ...
            moments accept a `datetime` or a bare `date`.
    ...
    async def search_gaps(
        ...
        *,
        ...
        :param from_: start of the search window: a datetime, a bare
            date (rendered as midnight UTC) or a ready-made API string.
            The current moment by default.
        :param to: end of the search window, in the same forms as
            `from_`. Must be strictly greater than `from_`.
    ...
    async def iter_gaps(
        ...
        *,
        ...
        :raises ValueError: If there is no employee at all or more than
            100 of them.
        """
        # a set or a generator is accepted, but the same collection is sent
        # for every page, so it is materialised once up front
        users = _check_sequence(users, "users", "logins or ids", "'login1'")
        page = 1
        while True:
