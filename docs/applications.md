...
`Application` — идентификатор (`id`), тип (`type`) и название (`name`) приложения.
В ответе `get_applications` значение `type` совпадает с `id`, но у приложения, вложенного
во внешнюю ссылку, тип может быть общим (например, `"app"`), поэтому не полагайтесь на это
равенство. Значение `id` понадобится для `add_remote_link` в качестве `origin`.
...
4. `relationship` — тип связи; документация API рекомендует использовать `"RELATES"`,
   поэтому это значение используется по умолчанию. Обратите внимание: для этого запроса
   документировано только значение в верхнем регистре. Перечисление `LinkRelationship`
   со значениями в нижнем регистре относится к связям между задачами и импорту
   (см. [«Импорт»](import.md)) и здесь не подходит.
