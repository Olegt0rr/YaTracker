...
5. `from_` — начало временного диапазона: `datetime` или готовая строка формата
   `YYYY-MM-DDThh:mm:ss.sss±hhmm` (query-параметр `from`). `datetime` приводится к этому
   формату; наивный `datetime` (без таймзоны) отправляется без смещения и вызывает
   `UserWarning` — передавайте timezone-aware значения.
